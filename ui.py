...
DEMO_SCHEDULE_FILE = DEMO_DIR / "demo_schedule.json"
DEMO_WBGT_FILE = DEMO_DIR / "demo_wbgt.json"
...
def clean_recommendation(text):
    """
    Remove the incorrect Task Delay section from the
    AI-generated explanation.

    Example removed section:

    **7. Task delay (minutes)**
    60 minutes total (each task delayed by 1 hour).
    """
    ...
    # Remove markdown section beginning with:
    # **7. Task delay (minutes)**
    #
    # and remove its following explanation line.
    text = re.sub(
        r"\*\*7\.\s*Task delay\s*\(minutes\)\*\*"
        r"\s*"
        r".*?"
        r"(?=\n\s*\*\*|\Z)",
        "",
    ...
    # Remove any remaining standalone sentence containing
    # "minutes total (each task delayed by 1 hour)"
    text = re.sub(
        r".*?minutes total\s*"
        r"\(each task delayed by 1 hour\)\.?",
        "",
        text,
        flags=re.IGNORECASE
    )
...
if run_agent:
    ...
    try:
        ...
        with st.spinner(
            ...
            # Remove incorrect task-delay section
    ...
    if not st.session_state.agent_ran:
        ...
        if st.session_state.plan_approved:
            ...
            try:
                ...
                final_delay_check = (
                    tools_module.check_plan_delay(
                        approved_plan,
                        schedule=schedule
                    )
                )

                final_project_delay = int(
                    final_delay_check.get(
            ...
            if final_project_delay > 0:

                st.warning(
                    f"Project delay: "
                    f"**{final_project_delay} min**"
                )
