...
def _normalise_plan_changes(plan):
    """
    Convert supported plan formats into an explicit
    task-ID -> change mapping.

    IMPORTANT:
    The task ID is ALWAYS the source of truth.

    This function deliberately avoids positional/index-based
    mapping. Therefore:
    ...
    # --------------------------------------------------------
    # CASE 1:
    # A direct RESCHEDULE plan
    #
    # This is intentionally checked FIRST.
    #
    # Example:
    #
    # {
    #     "plan_type": "RESCHEDULE",
    #     "task_id": "T2",
    #     "start": "10:00",
    #     "end": "12:00"
    # }
    ...
    # --------------------------------------------------------
    # CASE 4:
    # "proposed_changes" dictionary
    #
    # Used only as a fallback.
    #
    # We deliberately do NOT let this override an explicit
    # task_id or nested original plan.
    # --------------------------------------------------------
...
def find_safe_swaps(
    ...
    IMPORTANT:
    The supplied task_id is preserved throughout the entire
    ...
    for start in sorted(
        ...
        candidate = {
            ...
            # CRITICAL:
            # Preserve the original requested task ID.
            "task_id":
                str(task_id),
...
def find_redeployment_plans(
    ...
    The outdoor task ID and indoor task ID are explicitly
    preserved.
    """
...
# ============================================================
# APPROVED PLAN -> TASK CHANGES
# ============================================================

def _plan_to_task_changes(plan):
    ...
    # ========================================================
    #
    # A combined plan explicitly contains multiple task IDs.
    #
    # Example:
    #
    # {
    #     "plan_type": "COMBINED_PLAN",
    #     "task_ids": ["T1", "T2"],
    #     "proposed_changes": {
    #         "T1": {...},
    #         "T2": {...}
    ...
    #
    # IMPORTANT:
    # Check this BEFORE the nested "plan" logic.
    # ========================================================
    ...
    if plan.get(
        ...
        for task_id, change in proposed_changes.items():
            ...
            if (
                change.get("start") is None
                or
                change.get("end") is None
            ):
                continue
    ...
    original_plan = plan.get("plan")

    if isinstance(original_plan, dict):

        original_changes = _plan_to_task_changes(
            original_plan
        )
    ...
    if plan.get("plan_type") == "SWAP_WORKERS":
        ...
        if outdoor_id is not None:
            ...
            if not outdoor.get("start") or not outdoor.get("end"):
                raise ValueError(
                    f"Outdoor task {outdoor_id} is missing start/end."
                )
        ...
        indoor_id = indoor.get(
            "task_id"
        )

        if indoor_id is not None:

            indoor_id = str(
                indoor_id
            )

            if not indoor.get("start") or not indoor.get("end"):
                raise ValueError(
                    f"Indoor task {indoor_id} is missing start/end."
                )
    ...
    if (
        task_id is not None
        and
        plan.get("start") is not None
        and
        plan.get("end") is not None
    ):
    ...
    for key, value in plan.items():
        ...
        if (
            ...
            return {
                task_id: deepcopy(value)
            }

    # ========================================================
    # 6. proposed_changes FALLBACK
    # ========================================================
...
def compare_plans(
    ...
    for plan in plans:
        ...
        evaluated_plan = {
            ...
            # Preserve the ORIGINAL AI plan exactly.
            "plan":
                deepcopy(
                    plan
                ),
...
# ============================================================
# COMBINE BEST PLANS FOR ALL ACTION-REQUIRED TASKS
# ============================================================

def combine_best_plans(
    evaluated_plans,
    action_task_ids,
    schedule=None
):
    """
    Combine the best feasible individual plan for every
    action-required task into ONE final plan.

    Example:

        T1 best plan -> 10:00-12:00
        T2 best plan -> 10:00-12:00
    ...
    IMPORTANT:
    - Each task is selected independently.
    - Task IDs remain explicit.
    - The combined plan is validated again.
    - The final plan can be passed directly to
      update_schedule().
    """
    ...
    for task_id in action_task_ids:
        ...
        for evaluated in evaluated_plans:
            ...
            # ------------------------------------------------
            # Determine which task IDs this plan affects.
            # ------------------------------------------------

            affected_ids = set()

            # Direct task_id
            direct_task_id = evaluated.get(
                "task_id"
            )
            ...
            if isinstance(
                ...
                original_task_id = original_plan.get(
                    "task_id"
                )
                ...
                paired_task_id = original_plan.get(
                    "paired_task_id"
                )
                ...
                # SWAP_WORKERS
                outdoor_task = original_plan.get(
                    "outdoor_task",
                    {}
                )
            ...
            proposed_changes = evaluated.get(
                "proposed_changes",
                {}
            )
            ...
            if isinstance(
                ...
                for proposed_task_id in proposed_changes:

                    affected_ids.add(
                        str(proposed_task_id)
                    )
        ...
        # ----------------------------------------------------
        # No feasible plan for this task
        # ----------------------------------------------------

        if not task_candidates:

            # Every action-required task must have
            # a feasible plan before we create the
    ...
    # --------------------------------------------------------
    # If no task has a feasible plan
    # --------------------------------------------------------
    ...
    for evaluated_plan in selected_plans:
        ...
        for task_id, change in changes.items():
            ...
            # ------------------------------------------------
            # Prevent accidental duplicate task selection.
            # The first explicitly selected plan remains
            # authoritative.
            # ------------------------------------------------

            if task_id not in combined_changes:
    ...
    # This is important.
    #
    # We do NOT assume that individually safe plans are
    # automatically safe when combined.
    # --------------------------------------------------------

    combined_evaluation = compare_plans(
        [
            combined_plan
        ],
        schedule
    ...
    if not combined_evaluation:

        return None

    final_plan = combined_evaluation[0]

    # --------------------------------------------------------
    # Preserve useful information about the individual
    # decisions.
    # --------------------------------------------------------
    ...
    for evaluated_plan in selected_plans:
        ...
        original_plan = evaluated_plan.get(
            "plan",
            {}
        )
...
def update_schedule(
    ...
    """
    Apply an approved AI plan to the active schedule.

    The original plan's explicit task_id is authoritative.
    """
    ...
    if not isinstance(
        ...
        raise ValueError(
            "Approved plan must be a dictionary."
        )

    # ========================================================
    # EXTRACT EXACT TASK CHANGES
    # ========================================================
    ...
    if not changes:
        ...
        raise ValueError(
            "No valid task changes found in approved plan."
        )
    ...
    print("\n" + "=" * 60)
    print("APPROVED PLAN -> SCHEDULE UPDATE")
    print("=" * 60)
    ...
    for task_id, change in changes.items():
        ...
        if not start or not end:
            ...
            raise ValueError(
                f"Task {task_id} is missing start/end."
            )
    ...
    # ========================================================
    # APPLY CHANGES BY EXPLICIT TASK ID
    # ========================================================

    for task_id, change in validated_changes.items():
        ...
        schedule[task_id]["start"] = (
            change["start"]
        )

        schedule[task_id]["end"] = (
            change["end"]
        )
        ...
        if change.get("workers") is not None:

            schedule[task_id]["workers"] = list(
                change["workers"]
            )
    ...
    verified_schedule = load_schedule()

    for task_id, change in validated_changes.items():

        saved = verified_schedule.get(
            task_id,
            {}
        )

        if (
            saved.get("start") != change["start"]
            or
            saved.get("end") != change["end"]
        ):

            raise RuntimeError(
                f"Schedule verification failed for {task_id}. "
                f"Expected "
                f"{change['start']}-{change['end']}, "
                f"but saved "
                f"{saved.get('start')}-{saved.get('end')}."
            )
